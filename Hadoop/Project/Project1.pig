...
inputDialogues4 = LOAD '/user/jobinxraja/inputs/episodeIV_dialogues.txt' using PigStorage('\t') AS (name:chararray,line:chararray);
inputDialogues5 = LOAD '/user/jobinxraja/inputs/episodeV_dialogues.txt' using PigStorage('\t')  AS (name:chararray,line:chararray);
inputDialogues6 = LOAD '/user/jobinxraja/inputs/episodeVI_dialogues.txt' using PigStorage('\t')  AS (name:chararray,line:chararray);
...
rmf hdfs:///user/jobinxraja/Outputs;

-- Store the result in HDFS
STORE namesOrdered INTO 'hdfs:///user/jobinxraja/Outputs' using PigStorage('\t');